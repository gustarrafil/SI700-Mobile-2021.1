...
caminho(List<int> thor, List<int> martelo) {
  ...
  matriz.construir(thor, martelo);
  return matriz.converter();
}

List<List<int>> passagem(List<int> thor, List<int> martelo) {
  List<List<int>> listaModelo = [];
  if (thor[POSICAO_X] != martelo[POSICAO_X] &&
      thor[POSICAO_Y] != martelo[POSICAO_Y]) {
    while (thor[POSICAO_X] != martelo[POSICAO_X]) {
      if (thor[POSICAO_X] < martelo[POSICAO_X]) {
        thor[POSICAO_X] += ANDAR_UNIDADE;
        listaModelo.add(List.from(thor));
      } else {
        thor[POSICAO_X] -= ANDAR_UNIDADE;
        listaModelo.add(List.from(thor));
      }
    }
    while (thor[POSICAO_Y] != martelo[POSICAO_Y]) {
      if (thor[POSICAO_Y] < martelo[POSICAO_Y]) {
        thor[POSICAO_Y] += ANDAR_UNIDADE;
        listaModelo.add(List.from(thor));
      } else {
        thor[POSICAO_Y] -= ANDAR_UNIDADE;
        listaModelo.add(List.from(thor));
